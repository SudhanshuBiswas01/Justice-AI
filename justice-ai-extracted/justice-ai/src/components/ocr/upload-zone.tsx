...
export function UploadZone({
  ...
  onFiles: (files: { name: string; size: string }[]) => void
}) {
  ...
  return (
    ...
    >
      ...
      {/* Ambient grid + glow when dragging */}
      <AnimatePresence>
        {dragging && (
          <motion.div
            initial={ { opacity: 0 } }
      ...
      <p className="mt-1.5 max-w-sm text-sm text-muted-foreground">
        Drag &amp; drop contracts, notices or judgments — or click to browse.
        PDF, PNG, JPG and DOCX supported.
      </p>
    </motion.div>
  )
}
